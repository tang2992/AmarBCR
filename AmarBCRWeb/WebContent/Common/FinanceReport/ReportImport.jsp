...
//把增加的报表数据填充到excel中
	...
	sSql = " select RowName,RowNo,Col1Value,Col2Value from REPORT_DATA WHERE REPORTNO='"+sReportNo+"' order by DisplayOrder";
	//先通过count方式获取行数
	rs = Sqlca.getASResultSet("select count(*) as cnt from (" + sSql + ") tb");
	if (rs.next()) iRowCount = rs.getInt("cnt");
	rs.getStatement().close();
