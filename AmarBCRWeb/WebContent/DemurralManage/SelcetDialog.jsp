...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Tree01;Describe=定义页面属性;]~*/%>
	<%
	String PG_TITLE = "记录类型选择视图"; // 浏览器窗口标题 <title> PG_TITLE </title>
	String PG_CONTENT_TITLE = "&nbsp;&nbsp;记录类型选择&nbsp;&nbsp;"; //默认的内容区标题
...
<%/*~BEGIN~可编辑区~[Editable=true;CodeAreaID=Tree03;Describe=定义树图;]~*/%>
	<%
	String sReportType  = DataConvert.toRealString(iPostChange,(String)CurPage.getParameter("ReportType"));
	
	//定义Treeview
	HTMLTreeView tviTemp = new HTMLTreeView(SqlcaRepository,CurComp,sServletURL,"记录类型列表","right");
    tviTemp.TriggerClickEvent=true;
    String sSql = "";
	sSql = "select relativeCode from CODE_LIBRARY where CodeNo='ReportType' and ItemNo ='"+sReportType+"'";
	String sRecordType = Sqlca.getString(sSql);
	
	String sSqlTreeView = " from CODE_LIBRARY where CodeNo='"+sRecordType+"' and IsInUse = '1'";
	//参数从左至右依次为：
