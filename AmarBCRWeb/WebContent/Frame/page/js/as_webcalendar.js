...
function SelectDate(obj,strFormat,startDate,endDate,postEvent,x,y,isInlist){
	...
	cal.show(obj);
	cal.postEvent = typeof postEvent != "function" ? function(){/*return postEvent;*/} : postEvent;//添加后续事件
}
...
if(this.dateFormatStyle.indexOf('s')< 0) {this.DateMode =pickMode["minute"];}//精度为分
if(this.dateFormatStyle.indexOf('m')< 0) {this.DateMode =pickMode["hour"];}//精度为时
if(this.dateFormatStyle.indexOf('h')< 0) {this.DateMode =pickMode["day"];}//精度为日
if(this.dateFormatStyle.indexOf('d')< 0) {this.DateMode =pickMode["month"];}//精度为月
if(this.dateFormatStyle.indexOf('M')< 0) {this.DateMode =pickMode["year"];}//精度为年
if(this.dateFormatStyle.indexOf('y')< 0) {this.DateMode =pickMode["second"];}//默认精度为秒
...
"tr_word" : "#666", //日历头文字颜色
"tr_bg" : "#fff", //日历头背影色
"input_border" : "#d8d8d8", //input控件的边框颜色
...
mvAry[mvAry.length] = '<input style="border: 2px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:20px;height:18px;color:#333333;font-weight:bold" name="prevYear" type="button" id="prevYear" value="&lt;&lt;"></th>';//向左input
mvAry[mvAry.length] = '<th align="center" width="98%" nowrap="nowrap"><input style="border: 2px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:18px;color:#333333;font-weight:bold';//向左input
...
mvAry[mvAry.length] = '<input style="border: 2px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:18px;color:#333333;font-weight:bold';//向右input
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏“月”
mvAry[mvAry.length] ='" name="nextMonth" type="button" id="nextMonth" value="&gt;" /></th>';
mvAry[mvAry.length] = '<th align="right" width="1%"><input style="border: 2px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:20px;height:18px;color:#333333;font-weight:bold" name="nextYear" type="button" id="nextYear" value="&gt;&gt;">';//向右input
