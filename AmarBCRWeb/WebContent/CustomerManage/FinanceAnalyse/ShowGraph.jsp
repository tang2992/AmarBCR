...
<%
   //获得页面参数，报表名称、报表月份、报表值、图像类型
	String sItemNames     = CurPage.getParameter("ItemNames");
	String sMonthsScopes = CurPage.getParameter("MonthsScopes");//实现多口径后，要加上口径参数
	String sItemValues    = CurPage.getParameter("ItemValues");
	String sGraphType	  = CurPage.getParameter("GraphType");
    if (sItemValues==null || sItemValues.equals("")) {
    	sItemValues="0";
    }
    int iScreenWidth = 1024;
    int iScreenHeight = 768;
	String sScreenWidth	= CurPage.getParameter("ScreenWidth");
    if (sScreenWidth !=null && !sScreenWidth.equals("")) {
    	iScreenWidth = Integer.parseInt(sScreenWidth);
    }
	String sScreenHeight= CurPage.getParameter("ScreenHeight");
