...
<%
	...
	String sOrgLevel = DataConvert.toRealString(iPostChange,(String)CurComp.getParameter("OrgLevel"));
	if(sOrgLevel == null) sOrgLevel = "";
	else iLevel = Integer.valueOf(sOrgLevel).intValue();
...
<%	
	...
	String sSql = " select OrgID,OrgName,BankID,Status,OrgLevel,SortNo from ORG_INFO where  SortNo like '"+ sSortNo + "%' and  length(SortNo) ="+sSortNoLength.split("@")[iLevel]+" order by OrgID";

	ASDataObject doTemp = new ASDataObject(sSql);
	doTemp.setHeader(sHeaders);	
	doTemp.UpdateTable="ORG_INFO";
	doTemp.setKey("OrgID",true);
	doTemp.setVisible("OrgLevel",false);
...
<%/*~BEGIN~可编辑区~[Editable=false;CodeAreaID=List06;Describe=自定义函数;]~*/%>
	...
	function noRelativeRole()
    {
        ...
        { 
            alert(getHtmlMessage('1'));//请选择一条信息！
        }else if(confirm("只设置本机构，不涉及到其下级机构?")){
	...
	function mySelectRow()
	{
		...
		{
			OpenComp("GrantSubOrgList","/SystemManage/RightManage/GrantSubOrgList.jsp","UserID=<%=sUserID%>"+"&OrgID="+sOrgID+"&SortNo="+sSortNo+"&SortNoLength=<%=sSortNoLength%>","DetailFrame");
		}
	}		    
...
<script language=javascript>	
	...
	OpenPage("/Blank.jsp?TextToShow=请先选择相应的所属机构!","DetailFrame","");
</script>	
<%/*~END~*/%>
