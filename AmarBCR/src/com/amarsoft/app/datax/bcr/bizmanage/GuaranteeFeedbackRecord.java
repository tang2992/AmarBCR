...
public class GuaranteeFeedbackRecord
{
	...
	private static Record recordTemplete[] = new Record[1000];
	...
	public GuaranteeFeedbackRecord()
	{
		errorCount = 0;
		errorRow = 0;
		errorColumns = null;
	...
	public GuaranteeFeedbackRecord(Record feedbackRec)
		throws BCRException
	{
		errorCount = 0;
		errorRow = 0;
		...
		errorCount = segB.getField(2405).getString().length() / 8;
		errorRow = segB.getField(2403).getInt();
		errorColumns = new int[errorCount];
		errorCode = new String[errorCount];
		String s = segB.getField(2405).getString();
		...
		recordData = segB.getField(2406).getTextValue().getBytes();
	}
		...
		throws BCRException
	{
		...
		for (int i = 0; i < errorCount; i++)
			...
			} else
			{
				...
				if (rec == null)
					throw new BCRException((new StringBuilder("转换记录数据到记录对象失败: ")).append(recordData).toString());
				int offset = 0;
		...
		throws BCRException
	{
		if (messageSetXmlFiles == null || messageSetXmlFiles.length < 1)
			throw new BCRException("无效的配置XML文件");
