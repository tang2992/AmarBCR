...
public class MessageSet
{
		...
		throws BCRException
	{
		...
		{
			throw new BCRException("Build Document error", e);
		}
		Element xRoot = doc.getRootElement();
		Element xMessageList = xRoot.getChild("messageList");
		if (xMessageList == null)
			throw new BCRException((new StringBuilder("No message list define--")).append(defineFile).toString());
		List l = xMessageList.getChildren("message");
		if (l.size() == 0)
			throw new BCRException((new StringBuilder("No message  definition or error in messageList's attributes ")).append(defineFile).toString());
		MessageSet messageSet = new MessageSet(l.size());
		attribute = xMessageList.getAttribute("type");
		if (attribute == null)
			throw new BCRException((new StringBuilder("No TYPE  definition or error in messageList's attributes ")).append(defineFile).toString());
		messageSet.type = attribute.getValue();
		attribute = xMessageList.getAttribute("name");
		if (attribute == null)
			throw new BCRException((new StringBuilder("No NAME  definition or error in messageList's attributes ")).append(defineFile).toString());
		messageSet.name = attribute.getValue();
		attribute = xMessageList.getAttribute("direction");
		if (attribute == null)
			throw new BCRException((new StringBuilder("No direction   definition  or error in messageList's attributes ")).append(defineFile).toString());
		...
		{
			throw new BCRException((new StringBuilder()).append(exception.getMessage()).append(" @messageSet name:").append(messageSet.getName()).toString());
		}
		return messageSet;
	}
