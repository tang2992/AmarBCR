	...
	implements Cloneable
{
		...
		throws BCRException
	{
		...
		if (attribute == null)
			throw new BCRException("No segmentflag   definition  or errorin segment's attributes ");
		...
		if (attribute == null)
			throw new BCRException((new StringBuilder("No segmentlength   definition  or errorin segment's attributes  @ segmentFlag:")).append(segment.segmentFlag).toString());
		try
		{
		...
		{
			ARE.getLog().debug(e);
			throw new BCRException((new StringBuilder("No segmentlength   definition  or errorin segment's attributes  @ segmentFlag:")).append(segment.segmentFlag).toString(), e);
		}
		attribute = xSegment.getAttribute("optional");
		if (attribute == null)
			throw new BCRException((new StringBuilder("No optional   define  or error in segment's attributes  @ segmentFlag:")).append(segment.segmentFlag).toString());
		segment.optional = attribute.getValue(false);
		attribute = xSegment.getAttribute("unique");
		if (attribute == null)
			throw new BCRException((new StringBuilder("No unique   definition  or errorin segment's attributes  @ segmentFlag:")).append(segment.segmentFlag).toString());
		...
		{
			throw new BCRException((new StringBuilder()).append(exception.getMessage()).append(" @segment segmentFlag:").append(segment.getSegmentFlag()).toString());
		}
		return segment;
	}
