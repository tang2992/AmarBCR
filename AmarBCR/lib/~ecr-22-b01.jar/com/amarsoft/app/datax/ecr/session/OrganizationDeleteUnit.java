...
// Source File Name:   OrganizationDeleteUnit.java
...
public class OrganizationDeleteUnit extends ExecuteUnit
{
	...
	public int execute()
	{
		...
		ResultSet rs = null;
		String Sql = "select count(*) from HIS_batchDeleteOrgan where SessionID = '0000000000'";
		String Sql2 = "select count(*) from HIS_batchDeleteFamily where SessionID = '0000000000'";
		int iCount = 0;
		int iCount2 = 0;
		try
		{
		...
		for (int i = 0; i < (RecordNumberInMessage > 0 ? Math.max(iCount, iCount2) / RecordNumberInMessage : 1) + 1; i++)
		{
			try
			{
