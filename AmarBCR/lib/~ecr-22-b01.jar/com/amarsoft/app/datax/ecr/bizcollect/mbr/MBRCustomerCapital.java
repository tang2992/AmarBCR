...
// Source File Name:   MBRCustomerCapital.java
...
public class MBRCustomerCapital extends DBMessageBodyReader
{
	...
	public MBRCustomerCapital(Message message, String database)
	{
	...
	public MBRCustomerCapital(Message message)
	{
		super(message);
		recordSql = null;
		capiSql = null;
...
_L3:
		...
		try
		{
			...
			psKeeper.setString(2, occurDate);
			psKeeper.setString(3, customerId);
			...
			segB.getField(9991).setString(customerId);
...
_L2:
		...
		throws ECRException, SQLException
	{
		...
		psCapi.setString(2, occurDate);
		psCapi.setString(3, customerId);
		ResultSet rs;
		Segment segE;
		...
		throws ECRException, SQLException
	{
		...
		psInvest.setString(2, occurDate);
		psInvest.setString(3, customerId);
		ResultSet rs;
		Segment segF;
		...
		throws ECRException, SQLException
	{
		...
		psFamily.setString(2, occurDate);
		psFamily.setString(3, customerId);
		ResultSet rs;
		Segment segI;
	...
	public final void setRecordSql(String recordSql)
	{
		...
		capiSql = (new StringBuilder("select * from ")).append(capiTable).append(" where SessionID=? and OccurDate=? and CustomerID=?").toString();
		investSql = (new StringBuilder("select * from ")).append(investTable).append(" where SessionID=? and OccurDate=? and CustomerID=?").toString();
		keeperSql = (new StringBuilder("select * from ")).append(keeperTable).append(" where SessionID=? and OccurDate=? and CustomerID=?").toString();
		familySql = (new StringBuilder("select * from ")).append(familyTable).append(" where SessionID=? and OccurDate=? and CustomerID=?").toString();
	}
}
