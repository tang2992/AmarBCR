...
// Source File Name:   TransferFilter.java
	...
	implements RecordFilter
{
		...
		throws ECRException
	{
		...
		try
		{
			...
			String mb;
			String rt;
			for (rs = stmt.executeQuery("select MainBusinessNo,RecordScope,FilterCause from ECR_TRANSFERFILTER"); rs.next(); addFilter(rt, mb))
			{
				mb = rs.getString("MainBusinessNo");
				rt = rs.getString("RecordScope");
			}
			...
			if (ARE.getLog().isTraceEnabled())
			{
				...
				for (int i = 0; i < recordFilter.length; i++)
				{
					...
					if (recordFilter[i] == null)
					{
						sb.append("");
					} else
					{
						for (Iterator it = recordFilter[i].iterator(); it.hasNext(); sb.append(it.next()).append("|"));
					}
					sb.append("\n");
				}
		...
		{
			throw new ECRException("³õÊ¼»¯¹ýÂËÆ÷Ê§°Ü", e);
		}
