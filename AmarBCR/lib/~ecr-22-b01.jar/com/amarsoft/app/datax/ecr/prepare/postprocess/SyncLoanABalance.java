...
// Source File Name:   SyncLoanABalance.java
...
public class SyncLoanABalance extends ExecuteUnit
{
	...
	public int execute()
	{
		...
		try
		{
			logger.info("开始更新贷款合同可用余额...");
			String sqlQuery = "select BusinessSum,LContractNo,AvailabBalance,Recycle,IncrementFlag from ECR_LOANCONTRACT where LContractNo in(select LContractNo from ECR_LOANDUEBILL  where IncrementFlag='1' or IncrementFlag='2') ";
			String sqlUpdate = "update ECR_LOANCONTRACT set AvailabBalance=?,IncrementFlag=? where LContractNo=?";
			...
			while (rs.next()) 
			{
				String contractNo = rs.getString("LContractNo");
				...
				if (logger.isTraceEnabled())
				{
					...
					sb.append("ContactNo=").append(contractNo).append(",BusinessSum=").append(contractSum).append(",recycle=").append(recycle).append(",UsedSum=").append(usedSum).append(",OldAvaliableBalance=").append(availabBalance).append(",NewAvaliableBalance=").append(newAbalance).append(",IncrementFlag=").append(incrementFlag);
