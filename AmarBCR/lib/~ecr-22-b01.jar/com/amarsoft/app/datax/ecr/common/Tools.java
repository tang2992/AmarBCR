...
public class Tools
{
	...
	public static String changeID(String ID15)
	{
		...
		if (ID15 == null)
			return "";
		if (ID15.length() != 15 && ID15.length() != 18)
			return ID15;
		if (ID15.length() == 18)
	...
	protected static java.util.Date parseString2Date(String datestring, String format)
		throws Exception
	{
		java.util.Date date;
		String sDate = "";
		if (datestring.length() == 7)
			sDate = (new StringBuilder(String.valueOf(datestring))).append("/01").toString();
		else
			sDate = datestring;
		...
		Exception e;
		e;
		throw new Exception((new StringBuilder("×Ö·û'")).append(datestring).append("'×ª»»Òì³£").append(e).toString());
	}
