...
public class ECRDataSourceProvider extends DefaultDataSourceProvider
{
		...
		throws RecordSetException
	{
		...
		for (int i = 0; i < flds.length; i++)
			if (flds[i].getType() == 0)
			{
				...
				if (cm != null)
				{
					...
					if (StringX.parseBoolean(fp) && r.getField("CertType") != null)
		...
		if (table.equalsIgnoreCase("BANK_LOANCONTRACT"))
		{
		...
		if (table.equalsIgnoreCase("BANK_FACTORING"))
		{
			mainContractNoField = "FactoringNo";
			businessType = "2";
			fixGuarantyFlag = true;
		}
		if (table.equalsIgnoreCase("BANK_FINAINFO"))
		{
		...
		if (table.equalsIgnoreCase("BANK_GUARANTEEBILL"))
		{
			mainContractNoField = "GuaranteeBillNo";
			businessType = "6";
			fixGuarantyFlag = true;
		}
		if (table.equalsIgnoreCase("BANK_CREDITLETTER"))
		{
			mainContractNoField = "CreditLetterNo";
			businessType = "5";
			fixGuarantyFlag = true;
		}
		if (table.equalsIgnoreCase("BANK_ACCEPTANCE"))
		{
			mainContractNoField = "AcceptNo";
			businessType = "7";
			fixGuarantyFlag = true;
		}
		if (table.equalsIgnoreCase("BANK_LOANDUEBILL"))
			fixKind = true;
	}
				...
				if (f != null)
				{
					...
					if (v != null)
					{
						...
						if ((fieldsProcess[i][0] & 4) > 0)
							v = fixDigital(v);
						if ((fieldsProcess[i][0] & 2) > 0 && currentRecord.getField("CertType").getString().equalsIgnoreCase("0"))
							v = fixPID(v);
		...
		throws SQLException
	{
		...
		if (fieldName != null && fieldName.equalsIgnoreCase("LCONTRACTNO"))
		{
			...
			{
				String sql = "Select Sum(PutOutAmount) as PutOutAmount,Sum(Balance) as Balance FROM BANK_LOANDUEBILL where LCONTRACTNO=?";
		...
		} else
		{
			...
			{
				String sql = "Select Sum(PutOutAmount) as PutOutAmount,Sum(Balance) as Balance FROM BANK_FINADUEBILL where FCONTRACTNO=?";
		...
		throws SQLException
	{
		...
		{
			String sql = (new StringBuilder(" select count(Contractno) as GuarantyCount from BANK_ASSURECONT where Contractno= ?  and AvailabStatus='1' and BusinessType='")).append(businessType).append("'").append(" union select count(Contractno) as GuarantyCount from BANK_GUARANTYCONT where Contractno= ? and AvailabStatus='1' and BusinessType='").append(businessType).append("'").append(" union select count(Contractno) as GuarantyCount from BANK_IMPAWNCONT where Contractno= ? and AvailabStatus='1' and BusinessType='").append(businessType).append("'").toString();
